import { componentRecord, layer, Melooly, MeloolyLauncher, primaryColors, components } from "../index.js";
...
async function drawSample(melooly: Melooly, scale: number, items: Partial<componentRecord>, filename: string) {
	let fileType: 'svg' | 'png' = 'png'
	let canvas = new Canvas(270 * scale, 270 * scale, fileType == 'png' ? 'image' : 'svg');
	...
	writeFileSync(`./test/demoImages/${filename}.${fileType}`, drawOntoCanvas(canvas, melooly, scale, items))
	if (fileType == 'png') console.log(await terminalImage.file(`./test/demoImages/${filename}.png`, { width: 50, height: 50 }))
}
...
function generateReport(demos: Melooly[]) {
	...
	demos.forEach(demo => {
		...
		(Object.entries(demo.components) as [string, { color: string, value: string }][]).forEach(([layerName, data]) => {
			if (!stats[layerName]) {
...
\\f0\\fs24`;
	...
	const getValidOptions = (layerName: string): { values: string[], colors: string[] } => {
		...
		const key = layerName as keyof typeof components;
		if (components[key]) {
			// Ensure we are working with string[] for values
			const vals = components[key].values as any[];
	...
	for (const [layerName, layerData] of Object.entries(stats)) {
		...
		const unusedComps = validOptions.values.filter((v: string) => !usedComps.has(v));
		if (unusedComps.length > 0) {
			rtfContent += `\\tab\\b Unused:\\b0\\par\n`;
			unusedComps.forEach((u: string) => {
